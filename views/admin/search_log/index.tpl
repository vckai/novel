		...
		<table class="layui-table">
			...
			<tbody>
				...
				<tr>
					...
					<td class="td-manage">
						<a title="删除" href="javascript:;" onclick="log_del(this, '{{.Id}}', '{{.Kw}}')" style="text-decoration:none">
	...
	<script>
			...
			layui.use(['element', 'layer', 'laydate', 'laypage'], function() {
				...
				laypage({
					...
					jump: function (obj, first) {
						if (first != true) {
							window.location.href = {{urlfor "admin.SearchLogController.Index"}} + "?p=" + obj.curr + "&st={{.Search.st}}" + "&et={{.Search.et}}" + "&q={{.Search.q}}";
						}
					}
				}); 
		...
		function del_all() {
			layer.confirm('确认要删除吗？', function(index) {
				...
				//发异步删除数据
				ajax_post({{urlfor "admin.SearchLogController.DeleteBatch"}}, {ids: ids});
		    });
		}
		...
		function log_del(obj, id, kw) {
			layer.confirm('确认要删除吗？', function(index) {
				...
				//发异步删除数据
				ajax_post({{urlfor "admin.SearchLogController.Delete"}}, {id: id, kw: kw});
			});
		}
		</script>
</body>
